...
@Singleton
class RetrieveOtherCgtConnector @Inject() (val http: HttpClient, val appConfig: AppConfig)(implicit featureSwitches: FeatureSwitches)
  ...
  def retrieveOtherCgt(request: RetrieveOtherCgtRequest)(implicit
    ...
    val downstreamUri = if (taxYear.useTaxYearSpecificApi) {
      TaxYearSpecificIfsUri[RetrieveOtherCgtResponse](
        s"income-tax/income/disposals/other-gains/${taxYear.asTysDownstream}/${nino.value}"
      )
    } else {
      Api1661Uri[RetrieveOtherCgtResponse](
