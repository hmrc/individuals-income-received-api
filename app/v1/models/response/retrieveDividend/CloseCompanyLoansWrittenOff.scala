...
object CloseCompanyLoansWrittenOff {
  implicit val reads: Reads[CloseCompanyLoansWrittenOff] = Json.reads[CloseCompanyLoansWrittenOff]

  implicit val writes: OWrites[CloseCompanyLoansWrittenOff] = (
    (JsPath \ "customerReference").writeNullable[String] and
      (JsPath \ "grossAmount").write[BigDecimal]
    ) (unlift(CloseCompanyLoansWrittenOff.unapply))
}