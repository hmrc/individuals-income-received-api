...
import v1.services.{AmendDividendsService, AuditService, EnrolmentsAuthService, MtdIdLookupService}
        ...
        for {
          ...
          auditSubmission(
            GenericAuditDetail(
              ...
              AuditResponse(httpStatus = OK, response = Right(Some(Json.toJson(amendDividendsHateoasBody(appConfig, nino, taxYear)))))
            )
          )
