...
import v1.models.response.retrieveOther.{RetrieveOtherHateoasData, RetrieveOtherResponse}
...
class RetrieveOtherController @Inject()(val authService: EnrolmentsAuthService,
  ...
  def retrieveOther(nino: String, taxYear: String): Action[AnyContent] =
    authorisedAction(nino).async { implicit request =>
      ...
      implicit val desUri: DesUri[RetrieveOtherResponse] = DesUri[RetrieveOtherResponse](
      ...
      val result =
        for {
          ...
          serviceResponse <- EitherT(service.retrieve[RetrieveOtherResponse](parsedRequest))
