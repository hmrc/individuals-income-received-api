...
import v2.models.request.createAmendCgtPpdOverrides._
...
class CreateAmendCgtPpdOverridesValidator @Inject()(implicit appConfig: AppConfig)
  ...
  private def validateSinglePropertyDisposals(singlePropertyDisposals: SinglePropertyDisposals, arrayIndex: Int): List[MtdError] = {
    ...
    List(
      ...
      DateFormatValidation.validateOptional(Some(singlePropertyDisposals.completionDate),
        path = Some(s"/singlePropertyDisposals/$arrayIndex/completionDate")),
      ...
      DateFormatValidation.validateOptional(
        ...
        path = Some(s"/singlePropertyDisposals/$arrayIndex/acquisitionDate")),
      DecimalValueValidation.validate(
