...
import v1r6.models.response.retrieveForeign.{RetrieveForeignHateoasData, RetrieveForeignResponse}
...
class RetrieveForeignController @Inject()(val authService: EnrolmentsAuthService,
  ...
  def retrieveForeign(nino: String, taxYear: String): Action[AnyContent] =
    authorisedAction(nino).async { implicit request =>
      ...
      implicit val ifsUri: IfsUri[RetrieveForeignResponse] = IfsUri[RetrieveForeignResponse](
      ...
      val result =
        for {
          ...
          vendorResponse <- EitherT.fromEither[Future](
            hateoasFactory
              .wrap(serviceResponse.responseData, RetrieveForeignHateoasData(nino, taxYear))
