...
import api.models.errors.{MtdError, PpdSubmissionIdFormatError, RuleAmountGainLossError, RuleDuplicatedPpdSubmissionIdError, RuleIncorrectOrEmptyBodyError}
...
import v1r7.requestParsers.validators.validations.{DateFormatValidation, DecimalValueValidation, JsonFormatValidation, NinoValidation, PpdSubmissionIdValidation, TaxYearNotEndedValidation, TaxYearNotSupportedValidation, TaxYearValidation, ValueFormatErrorMessages, ValueGreaterThanValueValidation}
...
class CreateAmendCgtPpdOverridesValidator @Inject()(implicit currentDateTime: CurrentDateTime, appConfig: AppConfig)
  ...
  private def validateSinglePropertyDisposals(singlePropertyDisposals: SinglePropertyDisposals, arrayIndex: Int): List[MtdError] = {
    ...
    List(
      PpdSubmissionIdValidation.validate(singlePropertyDisposals.ppdSubmissionId,
