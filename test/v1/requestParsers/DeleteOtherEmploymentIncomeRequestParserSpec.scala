...
import v1.mocks.validators.MockDeleteOtherEmploymentIncomeValidator
...
class DeleteOtherEmploymentIncomeRequestParserSpec extends UnitSpec {
  val nino: String                   = "AA123456B"
  val taxYear: TaxYear               = TaxYear.fromMtd("2019-20")
  val taxYearString: String          = "2019-20"
  implicit val correlationId: String = "a1e8057e-fbbc-47a8-a8b4-78d9f015c253"

  private val rawData = DeleteOtherEmploymentIncomeRequestRawData(
    nino = nino,
    taxYear = taxYearString
  )

  trait Test extends MockDeleteOtherEmploymentIncomeValidator {

    lazy val parser: DeleteOtherEmploymentIncomeRequestParser = new DeleteOtherEmploymentIncomeRequestParser(
  ...
  "parse" should {
    "return a request object" when {
      "valid request data is supplied" in new Test {
        MockDeleteOtherEmploymentIncomeValidator
          .validate(rawData)
          .returns(Nil)

        val result: Either[ErrorWrapper, DeleteOtherEmploymentIncomeRequest] =
          parser.parseRequest(rawData)
        result shouldBe Right(DeleteOtherEmploymentIncomeRequest(Nino(nino), taxYear))
      }
    }
