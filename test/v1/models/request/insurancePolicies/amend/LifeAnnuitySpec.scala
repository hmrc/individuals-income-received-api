...
class LifeAnnuitySpec extends UnitSpec {

  "LifeAnnuity" should {
    "process Json correctly" when {
      "a full valid request is sent" in {
        fullLifeAnnuityJson.as[LifeAnnuity] shouldBe fullLifeAnnuityModel
      }

      "a minimal valid request is sent" in {
        minLifeAnnuityJson.as[LifeAnnuity] shouldBe minLifeAnnuityModel
      }
    }

    "write json correctly" when {
      "a full valid model is provided" in {
        Json.toJson(fullLifeAnnuityModel) shouldBe fullLifeAnnuityJson
      }

      "a  minimal valid model is provided" in {
        Json.toJson(minLifeAnnuityModel) shouldBe minLifeAnnuityJson
      }
    }
  }
}
