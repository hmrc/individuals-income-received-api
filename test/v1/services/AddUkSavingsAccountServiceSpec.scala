    ...
    "amendSaving" must {
      "return correct result for a success" in new Test {
        ...
        MockAddUkSavingsAccountConnector
          .addSavings(amendSavingsRequest)
          .returns(Future.successful(outcome))

        await(service.addSavings(amendSavingsRequest)) shouldBe outcome
      }
      ...
      "map errors according to spec" when {
        ...
        def serviceError(desErrorCode: String, error: MtdError): Unit =
          s"a $desErrorCode error is returned from the service" in new Test {
            ...
            await(service.addSavings(amendSavingsRequest)) shouldBe Left(ErrorWrapper(correlationId, error))
          }
