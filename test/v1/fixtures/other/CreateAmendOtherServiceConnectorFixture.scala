...
object CreateAmendOtherServiceConnectorFixture {

  val businessReceiptsModel: Seq[BusinessReceiptsItem] = Seq(
  ...
  val allOtherIncomeReceivedWhilstAbroadModel: Seq[AllOtherIncomeReceivedWhilstAbroadItem] = Seq(
  ...
  val overseasIncomeAndGainsModel = OverseasIncomeAndGains(gainAmount = 3000.99)

  val chargeableForeignBenefitsAndGiftsModel = ChargeableForeignBenefitsAndGifts(
  ...
  val omittedForeignIncomeModel = OmittedForeignIncome(amount = 4000.99)

  val requestBodyModel = CreateAmendOtherRequestBody(
