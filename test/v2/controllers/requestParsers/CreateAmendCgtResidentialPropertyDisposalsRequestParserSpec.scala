...
import v2.mocks.validators.MockCreateAmendCgtResidentialPropertyDisposalsValidator
...
class CreateAmendCgtResidentialPropertyDisposalsRequestParserSpec extends UnitSpec {
  ...
  "parse" should {
    ...
    "return an ErrorWrapper" when {
      ...
      "multiple field value validation errors occur when startDate and cessatonDate are outside of allowed range" in new Test {
        ...
        MockCreateAmendCgtResidentialPropertyDisposalsValidator
          .validate(createAmendCgtPpdOverridesRawData.copy(body = invalidValueRawBody))
          .returns(errors)

        parser.parseRequest(createAmendCgtPpdOverridesRawData.copy(body = invalidValueRawBody)) shouldBe
